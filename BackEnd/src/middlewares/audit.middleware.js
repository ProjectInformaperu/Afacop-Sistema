...
export function auditMiddleware(req, res, next) {
  ...
  res.on('finish', () => {
    const event = {
      ...
      actor: req.user?.username || 'anonymous', role: req.user?.rol || null,
      method: req.method, path: req.originalUrl.split('?')[0], statusCode: res.statusCode,
      ip: req.ip, userAgent: req.get('user-agent'), durationMs: Date.now() - startedAt,
    };
    logger.info(event, 'auditable_operation');
    const ipHash = crypto.createHash('sha256').update(req.ip || '').digest('hex');
    prisma.auditoriaSeguridad.create({ data: {
      request_id: req.id, actor_id: req.user?.id || null, actor: req.user?.username || 'anonymous',
      rol: req.user?.rol || null, metodo: req.method, ruta: req.originalUrl.split('?')[0].slice(0, 255),
      estado_http: res.statusCode, ip_hash: ipHash,
